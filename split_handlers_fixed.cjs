...
for (const file of toolFiles) {
    ...
    for (const elem of arrayLit.getElements()) {
        if (elem.isKind(SyntaxKind.ObjectLiteralExpression)) {
            // Check for string literal "function" or identifier function
            const funcProp = elem.getProperty("function") || elem.getProperty('"function"');
            if (funcProp && funcProp.isKind(SyntaxKind.PropertyAssignment)) {
                ...
                if (funcObj) {
                    const nameProp = funcObj.getProperty("name") || funcObj.getProperty('"name"');
